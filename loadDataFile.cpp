...
void LoadDataFile::initSheet(int sheetIndex, int patternCount)
{
	...
	for (int memberIndexCount = 0; memberIndexCount < _currentPattern->MemberIndexs->count(); memberIndexCount++)
	{
		...
		if (_currentPattern->TableType == 1)
		{
		...
		else if (_currentPattern->TableType == 0)
		{
			for (unsigned i = 0; i < indexCountF; i++)
			{
				indexNode->append(_currentPattern->MemberIndexs->at(memberIndexCount)->MemberDetailIndexs->at(i));
			...
			for (int resultIndex = 0; resultIndex < results->count(); resultIndex++)
			{
				...
				for (int formIndex = 0; formIndex < _currentResult->FormReuslts->count(); formIndex++)
				{
					...
					if (currentIR.at(0) == tableIndex)
					{
						...
						if (resultCollect.at(memberIndex).count() < indexCountGroup)
						{
						...
						{
							MemberIndex* tempMemberIndex = _currentPattern->MemberIndexs->at(i);
							for (int j = 0; j < tempMemberIndex->MemberDetailIndexs->count(); j++)
							{
								MemberDetailIndex* tempMemberDetail = tempMemberIndex->MemberDetailIndexs->at(j);
								...
								{
									indexGroup += tempMemberDetail->SecondLevelIndex.count();
								}
							}
						}
	...
	for (int patternCount = 0; patternCount < _info->RecognizePatternInfo->RecognizeFormPatterns->count(); patternCount++)
	{
		...
		if (_mExcelReader->newExcel(excelTempName))
		{
			...
			*/
				...
				if (currentIR.at(0) == tableIndex) //判断模式一致
				{
					...
					for (int unitIndex = 0; unitIndex < _currentPattern->EvaluationUnits.count(); unitIndex++)
					{
						...
						for (int subjectIndex = 0; subjectIndex < subjectCount; subjectIndex++)//主体组
						{
							...
							for (int formIndex = 0; formIndex < currentFormResults->count(); formIndex++)
							{
								...
								if (currentIR.at(2).toInt() == subjectIndex)
								{
									...
									if (_currentPattern->TableType == 0)
									{
										...
										{
											indexsShift += _currentPattern->FormPatternHash.at(i).size();
											for (int j = 0; j < _currentPattern->FormPatternHash.at(i).size(); j++)
											{
												MemberIndex* tempMember = _currentPattern->MemberIndexs->at(_currentPattern->FormPatternHash.at(i).at(j));
												for (int k = 0; k < tempMember->MemberDetailIndexs->count(); k++)
												{
													MemberDetailIndex* tempIndex = tempMember->MemberDetailIndexs->at(k);
													if (tempIndex->IndexLevel == 1)
													{
														groupShift++;
													}
													else if (tempIndex->IndexLevel == 2)
													{
									...
									for (int memberIndex = 0; memberIndex <  _currentPattern->FormPatternHash.at(page).size(); memberIndex++)
									{
										...
										if (_currentPattern->TableType == 0)
										{
											...
											currentMember = _currentPattern->MemberIndexs->at(0);
										}
										else
										{
										...
										if (_currentPattern->TableType == 0)
										{
											...
											for (int indexGroup = 0; indexGroup < _currentPattern->FormPatternHash.at(page).size(); indexGroup++)
											{
												currentMember = _currentPattern->MemberIndexs->at(indexGroup + indexsShift);
												for (int index = 0; index < currentMember->MemberDetailIndexs->count(); index++)
												{
							...
							receiveCount_unit.append(receiveCount_subject);
						}
						scoreCount.append(scoreCount_unit);
			...
			int pageCount = _currentPattern->GetFormPatternCount();
			bool lackPage = false;
			for (int unitIndex = 0; unitIndex < receiveCount.count(); unitIndex++)
			{
				QList<int> temp;
				for (int i = 0; i < receiveCount.at(unitIndex).at(0).count(); i++)
				{
					temp.append(0);
				}
				receiveCount[unitIndex].append(temp);
				for (int subjectIndex = 0; subjectIndex < receiveCount.at(unitIndex).count() - 1; subjectIndex++)
				{
						...
						*/
						...
						for (int memberIndex = 0; memberIndex < _currentMemberInfo->EvaluationMembers->at(unitIndex)->EvaluationMembers->count(); memberIndex++)
						{
							...
							if (subjectIndex == 0)
							{
								...
								_mExcelReader->writeExcel(excelRowCount + subjectIndex, 8, QString::number(receiveCount.at(unitIndex).at(subjectIndex).at(memberIndex)), format1);
							...
							if (subjectIndex == subjectCount - 1)
							{
								...
								_mExcelReader->writeExcel(excelRowCount + subjectCount, 8, QString::number(receiveCount.at(unitIndex).at(subjectCount).at(memberIndex)), format1);
